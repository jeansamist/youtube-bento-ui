  ...
  return (
    <div className="col-span-3 relative rounded-2xl overflow-hidden hover:scale-105 transition-transform">
      ...
      <div className="p-3 gap-2 flex w-full h-full z-10 relative backdrop-blur-3xl">
        <div className="absolute rounded-xl -translate-x-2 -translate-y-full inline-block right-0 -top-2 bg-[#09090B]/60 text-xs font-medium px-1.5">
          {duration}
        </div>
        <div>
...
export type ShortVideoCardProps = {
  image: StaticImageData;
  title: string;
  infos: string;
};

export const ShortVideoCard: FunctionComponent<ShortVideoCardProps> = ({
  image,
  infos,
  title,
}) => {
  return (
    <div className="col-span-2 relative rounded-2xl overflow-hidden hover:scale-105 transition-transform">
      <div
        className="absolute top-0 bottom-0 left-0 right-0 z-0 opacity-40"
        style={{
          background: `url("${image.src}") no-repeat center / cover`,
      ...
      ></div>
      <Image src={image} alt="" className="w-full relative z-10" />
      <div className="p-3 gap-2 w-full h-full z-10 relative backdrop-blur-3xl">
